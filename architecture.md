...
* **MEAT (`contracts/MEAT.sol`)** – mencetak token saat menerima mata uang native (menggunakan `DepositRate` per 1000 unit), menukar MEAT dengan GOAT, mengendalikan ketersediaan swap dan deposit rate, serta dapat menarik saldo native yang terkumpul.
...
Lingkungan Hardhat menjalankan proses deployment (`scripts/deploy.js`) dan pengujian di folder `test/`. Layanan backend dapat mengimpor kontrak terkompilasi dan memakai `ethers` untuk memanggil fungsi seperti `swapMEATForGOAT` atau `stake`.

## Frontend

Frontend berkomunikasi dengan kontrak yang telah dideploy menggunakan `ethers.js`, menyediakan alur untuk mencetak MEAT, melakukan swap antar token, serta staking dan klaim reward. Frontend mengandalkan alamat deployment dan artefak ABI yang dihasilkan Hardhat.
